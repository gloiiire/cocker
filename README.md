...
- Docker-compatible API socket — works with the `docker` CLI via `DOCKER_HOST=unix://~/.cocker/docker.sock`. Unimplemented endpoints answer 501 rather than guessing.
...
- Unit-tested with a CI-enforced 90 % line-coverage gate (see `docs/ROADMAP-1.0.md` for what that gate does and does not cover)
...
| **TTY allocation (`-t`)** | ✅ | ✅ | `run -it`, `exec -it`, `compose exec -it`, `attach` — real PTY, SIGWINCH resize, exit-code propagation |
| **`--privileged` / `--cap-add`** | ✅ | ✅ | propagated to cocker-init and applied to the bounding set before `setuid` |
...
(`man cocker.daemon.tls-init` — dot, not dash).
...
Cocker's socket advertises Docker Engine API v1.47 and implements a subset of it. The compatibility floor we commit to for the whole 1.x line is v1.41 — see `docs/ROADMAP-1.0.md`. You can use the standard `docker` CLI against a running `cockerd` instance:

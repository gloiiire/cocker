...
int main(int argc, char **argv) {
     ...
     */
    {
        ...
        /* Réécrit /etc/resolv.conf APRÈS udhcpc — udhcpc écrase ce fichier
         * avec le DNS du gateway DHCP (192.168.64.1), qui ne sait pas
         * résoudre les noms de containers cocker. On utilise la valeur passée
         * via cocker.dns dans le kernel cmdline (pointant vers le DNS interne
         * cocker côté host). */
        char *dns_ip = cmdline_get(cmdline, "dns");
        if (dns_ip) {
            int rc_fd = open("/etc/resolv.conf", O_WRONLY | O_CREAT | O_TRUNC, 0644);
            if (rc_fd >= 0) {
                /* Standard resolv.conf ne supporte pas les ports — on suppose
                 * que le DNS cocker écoute sur 53. Si ce n'est pas le cas, le
                 * trafic DNS interne ne marchera pas (voir docs port forwarding). */
                char rc_buf[512];
                int n = snprintf(rc_buf, sizeof(rc_buf),
                    "# generated by cocker-init (post-DHCP)\n"
                    "nameserver %s\n"
                    "options ndots:0\n",
                    dns_ip);
                if (n > 0) write(rc_fd, rc_buf, (size_t)n);
                close(rc_fd);
                info("rewrote /etc/resolv.conf → nameserver %s", dns_ip);
            }
            free(dns_ip);
        }
    }
